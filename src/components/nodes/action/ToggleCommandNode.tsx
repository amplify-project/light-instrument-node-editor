...
export function ToggleCommandNode({ data, id }: any) {
  ...
  useEffect(() => {
    ...
    if (data.registerConsumer) {
      data.registerConsumer(id, (incoming: any) => {
        if (incoming) {
          // If incoming value is 0, send a 'stop' command
          if (incoming.value == 0) {
            emitPayload({ device, port, command: "stop", value: "" });
          } else {
  ...
  const handleManualTrigger = (action: "press" | "release") => {
    return () => {
      const payload = (action == "press") ? (
        { device, port, command, value }
      ) : (
        { device, port, command: "stop", value: "" }
      );
  ...
  return (
    <div className="node command-node">
      ...
      <div className="node-header" title={"Converts a trigger signal into a structured command packet for the serial output. If a signal with value 0 is received, a 'stop' command is emitted. Any other value emits the command as specified.\nInput: Any signal\nOutput: Command packet {device, port, command, value}\nUse '#' in parameters to inject incoming value."}>
        <span>Toggle Command</span>
      ...
      <div className="node-content nodrag">
        ...
        <button disabled={command == ""} onMouseDown={handleManualTrigger("press")} onMouseUp={handleManualTrigger("release")}>
          Manual Trigger
        </button>
